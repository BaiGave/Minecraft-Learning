 ...
 *   or:   npm install  (from website/ directory)
 */
...
function build(showHeader) {
    ...
    try {
        // Step 1: Convert markdown articles
        execSync('node convert.js', { cwd: WEBSITE, stdio: 'pipe', shell: true });
        console.log(`  ${C_GREEN}✓${C_RESET} convert.js`);
...
if (chokidar) {
    ...
    watcher.on('all', (event, fp) => {
        log(`${C_YELLOW}${event}${C_RESET} ${path.relative(ROOT, fp)}`);
        scheduleBuild();
    });
