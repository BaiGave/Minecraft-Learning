  ...
  for (const entry of entries) {
    if (entry.isDirectory()) {
      ...
      const subPath = relativePath
        ...
        : entry.name;
      results.push(
        ...scanAllHtmlFiles(path.join(dir, entry.name), subPath, depth + 1),
      );
...
// ── 解析版本字符串 ───────────────────────────────
// 格式: {MC版本}-{加载器}-{模组版本}
// 示例: 1.21-core-, 1.21-neoforge-0.8.6, 1.21-fabric-1.7.3
function parseVersionString(versionStr) {
  if (!versionStr) return { mcVersion: "", loader: "", modVersion: "" };
...
/**
 * 从 docs/{模组}/ 下的一级版本目录名中选出默认版本键（与 converter 输出目录一致）。
 * 不能再用「仅 MC 版本号」（如 1.21.1），否则链接会指向不存在的 .../1.21.1/index.html。
 *
 * @param {string} moduleDir docs 下该模组的绝对路径
 * @param {string[]} versionKeys 一级子目录名列表
 * @param {string|null|undefined} preferred site-config 中的 defaultVersion（若存在且有效则优先）
 */
function pickDefaultVersionKey(moduleDir, versionKeys, preferred) {
  if (!versionKeys || versionKeys.length === 0) return null;
  const withIndex = versionKeys.filter((k) =>
    fs.existsSync(path.join(moduleDir, k, "index.html")),
  );
  if (preferred && versionKeys.includes(preferred)) {
    if (withIndex.includes(preferred)) return preferred;
  }
  if (withIndex.length > 0) {
    withIndex.sort((a, b) => b.length - a.length || a.localeCompare(b));
    return withIndex[0];
  }
...
function scanDocsDir() {
  ...
  for (const entry of entries) {
    ...
    // 扫描版本目录（支持新旧两种格式）
    // 新格式: 1.21-core-, 1.21-neoforge-0.8.6
    // 旧格式: 1.21, 1.21-neoforge
    const versionDirs = fs
      .readdirSync(moduleDir, { withFileTypes: true })
      .filter(
        (e) => e.isDirectory() && !e.name.startsWith(".") && e.name !== "root",
      )
      .filter((e) => /^\d+\.\d+/.test(e.name) || isVersionDirLegacy(e.name));
    ...
    const versionKeys = versionDirs.map((e) => e.name);
    ...
    if (isVersioned) {
      ...
      for (const vDir of versionDirs) {
        ...
        // 收集 MC 版本号（去重），用于首页「MC 版本」统计
        const { mcVersion } = parseVersionString(vDir.name);
        if (mcVersion && !mcVersions.includes(mcVersion)) {
          mcVersions.push(mcVersion);
        }
      ...
      // 默认版本：必须是实际一级子目录名（含 loader-模组版本），与 getDefaultVersionUrl 拼接一致
      defaultVersion = pickDefaultVersionKey(
        moduleDir,
        versionKeys,
        configModule?.defaultVersion,
      );
      if (!defaultVersion && versionKeys.length > 0) {
    ...
    modules[moduleId] = {
      ...
      colorGradient:
        configModule?.colorGradient ||
        "linear-gradient(135deg, #5B8C5A 0%, #6FA070 100%)",
      description: configModule?.description || `${moduleId} 相关文档`,
      isVersioned,
      /** 去重后的 MC 版本号，用于站点级统计 */
      versions: isVersioned ? mcVersions : null,
      /** 一级版本目录名列表（与 docs 下文件夹一致） */
      versionKeys: isVersioned ? versionKeys : null,
      defaultVersion,
      tutorials,
      analysis,
...
// 兼容旧格式的版本检测
function isVersionDirLegacy(name) {
  return /^\d+\.\d+(\.\d+)?(-[a-z]+)?$/i.test(name);
}
...
function buildAutoSiteModuleEntry(id, sm, order) {
  const entry = {
    ...
    colorGradient:
      sm.colorGradient ||
      "linear-gradient(135deg, #2a2a2a 0%, #525252 55%, #737373 100%)",
    accentColor: "#171717",
    description:
  ...
  const versionKeysForEntry =
    sm.versionKeys && sm.versionKeys.length ? sm.versionKeys : sm.versions;
  if (
    sm.isVersioned &&
    sm.defaultVersion &&
    versionKeysForEntry &&
    versionKeysForEntry.length
  ) {
    entry.versions = {};
    for (const v of versionKeysForEntry) {
      entry.versions[v] = {
        name: v,
        codename: "",
...
${AUTO_MODULES_END}`;
      source = source.replace(
        new RegExp(
          `${escapeRegExp(AUTO_MODULES_START)}[\\s\\S]*?${escapeRegExp(AUTO_MODULES_END)}`,
        ),
        emptyBlock,
      );
...
function writeSiteStatsJson(stats) {
  fs.writeFileSync(
    path.join(ROOT, "site-stats.json"),
...
function generateBasicIndex(moduleId, moduleData) {
  const relativePath = "../";
  const { name, icon, color, colorGradient, description, tutorials, analysis } =
    moduleData;
...
function main() {
  ...
  for (const [id, mod] of Object.entries(modules)) {
    console.log(
      `  • ${id}: ${mod.name} (${mod.tutorials + mod.analysis} 篇文档)`,
    );
  }
  ...
  console.log(
    `  MC 版本: ${stats.mcVersions.length > 0 ? stats.mcVersions.join(", ") : "无"}\n`,
  );
}
