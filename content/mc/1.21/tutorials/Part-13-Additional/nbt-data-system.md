...
学完本章节后，你将理解：
- NBT 是什么（命名二进制标签）
- NBT 的各种数据类�?- `NbtCompound` 复合标签的用�?- `NbtList` 列表标签的用�?- NBT �?Minecraft 中的实际应用

## 前置知识

- 理解 [Java 基础](/mc/1.21/tutorials/Part-0-Prerequisites/01-java-basics/)
- 了解基本的类、接口和集合概念
- 知道 Minecraft 的存档和物品系统是什�?
## 核心概念（用生活比喻�?
### 什么是 NBT�?
**NBT = Named Binary Tag（命名二进制标签�?*

简单来说，NBT 就是 Minecraft 用来存储数据的一种格式。就像你写日记需要纸和笔，Minecraft 存储数据就需�?NBT�?
### 生活中的比喻：NBT 就像...

想象你搬家时用的**收纳箱系�?*�?
```
📦 收纳箱（Compound 复合标签�?├── 📋 标签：物品信�?�?  ├── 名称：钻石剑
�?  ├── 耐久�?00/1561
�?  └── 附魔：锋�?III
├── 👕 标签：服�?�?  ├── 上衣：T�?�?  └── 裤子：牛仔裤
└── 📚 标签：书�?    ├── �?本：《Java入门�?    ├── �?本：《Minecraft模组开发�?    └── �?本：《数据结构�?```

每个收纳箱里有很�?*标签**（tag），每个标签都有自己�?*名字**�?*内容**�?
### NBT 数据类型一�?
| 类型 | ID | 说明 | 生活中的例子 |
|------|-----|------|-------------|
| **END** | 0 | 结束标记（很少用�?| - |
...
| **LONG_ARRAY** | 12 | 长整数数�?| UUID�?.16+�?|

### NbtCompound 复合标签

**NbtCompound** 就像一�?*字典**�?**JSON 对象**�?
```
{
    "display": {
        "Name": "§6黄金�?,
...
**NbtList** 就像一�?*数组**�?**List 集合**�?
```
[1, 2, 3, 4, 5]           // 数字列表
["�?, "�?, "�?]         // 字符串列�?[{id:1, lvl:1}, {id:2, lvl:2}]  // 复合标签列表
```
...
flowchart TD
    subgraph NBT["NBT 数据体系"]
        ...
        subgraph 容器类型
            NBTC[NbtCompound<br/>复合标签<br/>🔑�?值对🔑]
...
// 2. 写入各种类型的数�?nbt.putString("name", "钻石�?);        // 字符�?nbt.putInt("damage", 100);              // 整数
nbt.putInt("maxDamage", 1561);          // 整数
nbt.putBoolean("unbreakable", true);   // 布尔值（存为BYTE�?nbt.putFloat("speed", 3.5f);           // 浮点�?nbt.putLong("uuidMost", 123456789L);    // 长整�?nbt.putLong("uuidLeast", 987654321L);   // 长整�?
// 3. 写入嵌套复合标签
NbtCompound display = new NbtCompound();
display.putString("Name", "§6附魔钻石�?);
display.putString("Lore", "[\"传说�?..\",\"增加生命\"]");
nbt.put("display", display);

// 4. 读取数据
String name = nbt.getString("name");           // "钻石�?
...
// 3. 添加字符�?NbtList stringList = new NbtList();
stringList.add(NbtString.of("�?));
stringList.add(NbtString.of("�?));
stringList.add(NbtString.of("�?));

// 4. 添加复合标签到列表（用于附魔列表�?NbtList enchants = new NbtList();
...
enchant2.putShort("id", (short)20);  // 亡灵杀�?enchant2.putShort("lvl", (short)2);  // 亡灵杀手II
enchants.add(enchant2);

// 5. 读取列表
int first = list.getInt(0);        // 获取第一个元�?int size = list.size();            // 列表大小
...
// UUID 操作（Minecraft 1.16+�?UUID uuid = UUID.randomUUID();
...
target.copyFrom(source);  // 合并�?target
```
...
// 检查元素类�?NbtElement element = nbt.get("key");
...
public static NbtCompound createEnchantedBook() {
    ...
    // 附魔信息存储�?StoredEnchantments �?    NbtList enchants = new NbtList();
...
public static NbtCompound createItemWithLore(String name, String... lore) {
    ...
    // 显示信息
    NbtCompound display = new NbtCompound();
    display.putString("Name", name);  // 可以使用彩色代码�?"§6金苹�?
...
1. **NBT** = Minecraft 的数据存储格式，类似 JSON 但是二进�?2. **NbtElement** = 所�?NBT 类型的基类接�?3. **NbtCompound** = 键值对集合，类�?JSON 对象，用于存储复杂数�?4. **NbtList** = 有序列表，类似数组，用于存储列表数据
5. NBT 用于：世界存档、物品数据、方块实体数据、网络传�?6. **核心操作**：`put` 写入，`get` 读取，`contains` 检查，`remove` 删除
7. **安全访问**：优先使用类型安全的方法�?`getInt()`、`getString()`

## 练习

1. **基础练习**：创建一�?NbtCompound，包含玩家的基本信息（名字、等级、金币数量）
2. **进阶练习**：创建一个附魔列表，包括 3 种不同的附魔
3. **实战练习**：编写一个函数，将物品的 NBT 数据打印成易读的格式
4. **思考题**�?   - NBT �?JSON 有什么区别？
   - 为什�?Minecraft 要用二进制格式而不是纯文本�?5. **挑战练习**：创建一个递归函数，遍历任意嵌套的 NBT 结构并打印所有键值对
...
**继续学习**：下一章节将介�?[数据包系统](/mc/1.21/tutorials/Part-8-Resource/41-datapack-intro/)，它使用 JSON 格式来定义游戏内容，�?NBT 密切相关�?