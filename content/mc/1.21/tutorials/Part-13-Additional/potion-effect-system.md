...
学完本教程后，你将能够：
- 理解 Minecraft 药水效果系统的核心概�?- 掌握 StatusEffect、StatusEffectInstance 的使�?- 学会创建自定义药水效�?- 了解效果叠加规则和实现原�?
## 前置知识

- Java 基础（类、抽象类、枚举）
- [实体系统](/mc/1.21/tutorials/Part-4-Entity/20-entity-intro/) - 药水效果作用于实�?- [属性系统](/mc/1.21/tutorials/Part-4-Entity/24-entity-attributes/) - 效果可以修改属�?- [粒子系统](./particle-system.md) - 效果会显示粒�?
## 核心概念

### 什么是药水效果系统�?
想象你在�?RPG 游戏�?- **Buff** = 正面效果（如攻击增强、移动加速）
- **Debuff** = 负面效果（如中毒、减速）

Minecraft 的药水效果系统就像一�?**状态监控系�?*。当你喝下药水或被女巫扔药水时，游戏中会添加一个效果（StatusEffect）到你的身上。这个效果会持续一段时间（Duration），期间可能改变你的属性、移动速度或生成粒子�?
### 生活比喻：药水效�?= 生病时吃的药
...
| 药效等级（普通、加强） | amplifier�?=一级，1=二级�?|
| 药效持续时间 | duration�?00 tick = 30秒） |
| 吃药后身体变化（退烧、止痛） | 属性修改、伤害、粒子效�?|
| 药效退�?| 效果消失 |

### 效果系统的组�?
```
┌─────────────────────────────────────────────────────────�?�?                   药水效果系统组成                         �?├─────────────────────────────────────────────────────────�?�?                                                        �?�?  StatusEffect (效果定义)                                �?�?     ├── 颜色、分类、图�?                               �?�?     ├── 属性修改器                                      �?�?     └── 持续效果更新逻辑                                �?�?                                                        �?�?  StatusEffectInstance (效果实例)                        �?�?     ├── 持续时间、等�?                                 �?�?     ├── 粒子显示、叠加规�?                             �?�?     └── 隐藏的上一个效�?                               �?�?                                                        �?�?  LivingEntity (应用主体)                                �?�?     ├── activeStatusEffects (效果列表)                  �?�?     └── addStatusEffect() / removeStatusEffect()      �?�?                                                        �?└─────────────────────────────────────────────────────────�?```
...
classDiagram
    ...
    note for StatusEffect "相当�?药品种类'"
    note for StatusEffectInstance "相当�?实际吃的�?"
```
...
sequenceDiagram
    ...
    loop 每tick (0.05�?
        ...
        SE->>SE: 检查是否可以更�?        SE->>LE: applyUpdateEffect()
        LE->>LE: 修改属�?造成伤害
    end
...
public class StatusEffect implements ToggleableFeature {
    ...
    // 每tick更新时调用，返回false会移除效�?    public boolean applyUpdateEffect(LivingEntity entity, int amplifier) {
        return true;  // 默认不做任何�?    }
...
public class StatusEffectInstance implements Comparable<StatusEffectInstance> {
    ...
    private int duration;                   // 持续时间（tick�?    private int amplifier;                 // 等级�? = I级）
...
public class PoisonStatusEffect extends StatusEffect {
    ...
    @Override
    public boolean canApplyUpdateEffect(int duration, int amplifier) {
        // �?25 tick（约 1.25 秒）造成一次伤�?        int tickInterval = 25 >> amplifier;  // 等级越高间隔越短
    ...
    public boolean applyUpdateEffect(LivingEntity entity, int amplifier) {
        if (entity.canTakeDamage()) {
            // 造成 1 点伤害（等级越高伤害越高�?            entity.damage(
                DamageSource.MAGIC, 
                1.0f << amplifier
            );
        }
...
public class RegenerationStatusEffect extends StatusEffect {
    ...
    @Override
    public boolean canApplyUpdateEffect(int duration, int amplifier) {
        // �?50 tick（约 2.5 秒）治疗一�?        int tickInterval = 50 >> amplifier;
        return duration % tickInterval == 0;
    }
...
```java
// 方法1：使用效果实�?public void giveSpeedEffect(PlayerEntity player) {
...
// 方法2：快捷方法（省略实例创建�?public void giveStrengthEffect(PlayerEntity player) {
    player.addStatusEffect(
        StatusEffects.STRENGTH,
        1200,  // 持续 60�?        1      // 等级 1 (II�?
    );
}
```
...
// 清除所有负面效�?public void curePlayer(PlayerEntity player) {
...
### 场景 4：药水物�?
```java
// 创建自定义药水物�?public class FireAuraPotion extends PotionItem {
    ...
    public ItemStack finishUsing(ItemStack stack, World world, LivingEntity user) {
        ...
        // 播放喝药水音�?        world.playSound(null, user.getX(), user.getY(), user.getZ(),
            SoundEvents.ENTITY_GENERIC_DRINK, 
            SoundCategory.PLAYERS, 1.0f, 1.0f
        );
        
        // 消耗药�?        return PotionItem.useChargedItem(stack, world, user);
    }
}
```
...
public void giveBeaconEffect(PlayerEntity player) {
    StatusEffectInstance effect = new StatusEffectInstance(
        StatusEffects.SPEED,
        1800,   // 90�?        1,      // II�?        true,   // ambient = true（环境效果，粒子更透明�?        true,   // showParticles
        true    // showIcon
    );
    player.addStatusEffect(effect);
}
...
规则 1: 高等级效果替换低等级效果
  如果喝下 II 级速度时已经有 I 级：
    �?II 级速度替换 I �?    �?�?II 级结束时，自动恢�?I 级（如果有残留时间）

规则 2: 同等级刷新时�?  如果喝下 II 级速度时已经有 II 级：
    �?时间刷新为新的持续时�?
规则 3: 不同效果互不影响
  速度 + 跳跃提升 = 可以同时存在
```
...
### 效果冲突检�?
```java
// 检查是否会被替�?public boolean willEffectBeReplaced(LivingEntity entity, StatusEffect effect, int amplifier) {
...
| INSTANT_HEALTH | 正面 | 瞬间恢复 4 心每�?|
| INSTANT_DAMAGE | 负面 | 瞬间伤害 6 心每�?|
| JUMP_BOOST | 正面 | 跳跃高度 +0.5 格每�?|
| NAUSEA | 负面 | 屏幕扭曲 |
| REGENERATION | 正面 | �?2.5 秒恢�?0.5 �?|
| POISON | 负面 | �?1.25 秒伤�?0.5 �?|
| WITHER | 负面 | 类似中毒但致�?|
...
| 概念 | 作用 | 生活比喻 |
|------|------|----------|
| StatusEffect | 定义效果类型和行�?| 药品说明�?|
| StatusEffectInstance | 效果的具体实�?| 实际吃掉的药 |
| StatusEffectCategory | 效果分类（正�?负面/中性）| 药品颜色标签 |
| amplifier | 效果等级�?=一级）| 药量加�?|
| duration | 持续时间 | 药效持续多久 |

**核心要点�?*
1. StatusEffect 定义"什么效�?，StatusEffectInstance �?应用的效�?
2. 效果等级�?0 开始（0 = I级，1 = II级，以此类推�?3. 高等级效果会替换低等级，低等级会被隐藏但不消�?4. 效果通过 `LivingEntity.addStatusEffect()` 添加
5. 即时效果重写 `applyInstantEffect()`，持续效果重�?`applyUpdateEffect()`
...
### 练习 2：自定义负面效果
创建一�?饥饿"效果，使玩家的饥饿值快速下降�?
### 练习 3：火焰光环效�?创建一个光环效果，附近有敌人时自动对敌人造成火焰伤害�?
### 练习 4：效果检�?创建一个命令，可以显示玩家当前所有药水效果的详细信息�?
## 相关链接

### 内部链接
- [实体系统](/mc/1.21/tutorials/Part-4-Entity/20-entity-intro/) - 药水效果应用于实�?- [属性系统](/mc/1.21/tutorials/Part-4-Entity/24-entity-attributes/) - 效果修改实体属�?- [粒子系统](./particle-system.md) - 效果显示彩色粒子
- [物品系统](/mc/1.21/tutorials/Part-3-Block-Item/17-item-basics/) - 药水是特殊的物品
