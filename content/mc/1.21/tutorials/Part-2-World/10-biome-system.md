# 10 - 生物群系系统：Biome 的多彩世�?
## 目标

学完本章后，你将理解�?- Biome 类是什么，它包含哪些信�?- 温度和降雨如何决定生物群�?- 生物群系是如何存储在 Chunk 中的
- 生物群系在游戏中的实际应�?
## 前置知识

- [09-区块系统.md](./09-chunk-system.md) - 理解 Chunk 的结�?- [04-注册表系�?md](/mc/1.21/tutorials/Part-1-Foundation/04-registry-system/) - 理解注册表机�?
## 核心概念

### 生物群系是什么？

想象你在中国旅游�?
- **东北哈尔�?* �?冬天很冷，下雪，有冰�?- **海南三亚** �?常年温暖，海滩，椰子�?- **西藏高原** �?空气稀薄，草原，牦�?
**Minecraft 的生物群�?* = 世界各地�?气候区"在游戏里的实现！

每个生物群系决定了：
- 地表的方块（草地、沙子、雪�?- 植被（树木、花、草�?- 天气（是否下雨、下雪）
- 可以生成的生物（猪、牛、羊只在草原�?
```mermaid
flowchart LR
...
public final class Biome {
    ...
    // 生成设置（矿石、树木等特征�?    private final GenerationSettings generationSettings;

    // 出生设置（什么生物在这里生成�?    private final SpawnSettings spawnSettings;

    // 视觉效果（天空颜色、粒子、声音等�?    private final BiomeEffects effects;
}
```
...
record Weather(
    ...
    TemperatureModifier modifier, // 温度修正�?    float downfall              // 降雨�?(0.0 - 1.0)
)
```
...
public Precipitation getPrecipitation(BlockPos pos) {
    ...
    // 温度低于某个阈值就是雪，否则是�?    return this.isCold(pos) ? Precipitation.SNOW : Precipitation.RAIN;
}
```
...
private float computeTemperature(BlockPos pos) {
    ...
    // 高处温度下降（海拔效应）
    if (pos.getY() > 80) {
        // 添加噪声来调节温度变�?        float g = (float)(TEMPERATURE_NOISE.sample(
            (float)pos.getX() / 8.0f,
            (float)pos.getZ() / 8.0f,
            false
        ) * 8.0);
...
public boolean canSetIce(WorldView world, BlockPos pos, boolean doWaterCheck) {
    // 1. 不能是夏季生物群�?    if (this.doesNotSnow(pos)) {
        return false;
    }
    ...
    if (pos.getY() >= world.getBottomY() && pos.getY() < world.getTopY()
        ...
        if (fluidState.getFluid() == Fluids.WATER
            ...
            // 4. 周围也要有水（角落可能不结冰�?            boolean bl = world.isWater(pos.west())
...
public int getGrassColorAt(double x, double z) {
    // 从效果设置获取颜�?    int baseColor = this.effects.getGrassColor()
        .orElseGet(this::getDefaultGrassColor);

    // 根据温度和降雨调整颜�?    return this.effects.getGrassColorModifier()
...
### 生物群系的应用场�?
1. **地表生成**（见 11-地形生成.md�?   - 草地颜色
   - 地表方块（草方块 vs 沙砾 vs 沙子�?
2. **实体生成**
   ```java
   // 在什么生物群系可以生成什么生�?   Pool<SpawnSettings.SpawnEntry> getEntitySpawnList(
...
3. **方块放置**
   - 积雪冻原的村庄不会生成（太冷�?   - 丛林会生成藤�?
4. **方块特�?*
   - 下界不会生成冰（岩浆更多�?   - 冰刺高原有特殊的冰结�?
## 小结

| 概念 | 说明 |
|------|------|
| Biome | 表示一个气候区的类 |
| Temperature | 温度参数，决定是否下�?|
| Downfall | 降雨参数，影响湿�?|
| GenerationSettings | 该生物群系如何生成地形特�?|
| SpawnSettings | 该生物群系可以生成什么生�?|
| BiomeEffects | 视觉效果（颜色、粒子、声音） |

**关键理解**�?- **温度 + 降雨 = 生物群系**：这两个参数决定了具体的生物群系
- **高度影响温度**：高海拔地区温度更低
- **生物群系是分层的**：生成特征、出生设置、视觉效果各司其�?
## 练习

### 思考题

1. 为什么在 Minecraft 中山顶可能是雪，而山脚是森林�?2. 如果你要添加一个新�?火山"生物群系，它应该有什么特点？
3. 生物群系和矿石生成有什么关系？

### 动手�?
在源码中找到以下内容�?
1. 查看 `Biome.java` �?Biome 类的字段
2. 查看 `Biome.java` �?Weather record 的定�?3. 理解 `Biome.java` 中温度计算逻辑
...
| BiomeKeys.java | `net/minecraft/world/biome/BiomeKeys.java` | 生物群系�?|
| BiomeRegistry.java | `net/minecraft/registry/BiomeRegistry.java` | 生物群系注册�?|

## 相关链接

- [11-地形生成.md](./11-terrain-gen.md) - 生物群系如何影响地形生成
- [09-区块系统.md](./09-chunk-system.md) - 生物群系如何存储�?Chunk �?- Minecraft Wiki: [Biome](https://minecraft.wiki/w/Biome)
