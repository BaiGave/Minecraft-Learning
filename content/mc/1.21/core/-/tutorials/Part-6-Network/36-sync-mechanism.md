...
- 理解 Serverbound 和 Clientbound 的区别
...
- 每个人都看到相同的画面
- 一个人的操作，其他人也能看到
- 如果不同步，就会出现"你看到我在A点，但实际上我在B点"的情况
...
```mermaid
flowchart TB
    subgraph Server["服务端 (权威数据源)"]
        ServerWorld["服务端世界"]
        ServerPlayer["服务端玩家实体"]
        ServerEntities["服务端实体列表"]
        ServerChunks["服务端区块数据"]
    end
    ...
    subgraph Client["客户端 (本地渲染)"]
        ClientWorld["客户端世界"]
        ClientPlayer["客户端玩家实体"]
        ClientEntities["客户端实体列表"]
        ClientChunks["客户端区块缓存"]
    end

    ServerPlayer -->|"玩家移动"| MovePackets
    MovePackets -->|"验证"| ServerWorld
    ServerWorld -->|"实体状态"| EntityPackets
...
sequenceDiagram
    ...
    rect rgb(255, 230, 200)
        ...
        S->>S: 更新服务端玩家位置
        S->>S: 广播给周围玩家
        S-->>C: PlayerPositionLookS2CPacket
...
public class PlayPackets {
    // ===== 客户端 -> 服务端 =====
    // 玩家移动（完整位置+旋转）
    public static final PacketType<PlayerMoveC2SPacket.Full> MOVE_PLAYER_POS_ROT = 
    ...
    // 玩家移动（仅旋转）
    public static final PacketType<PlayerMoveC2SPacket.LookAndOnGround> MOVE_PLAYER_ROT = 
...
public class PlayPackets {
    // 区块批次开始
    public static final PacketType<StartChunkSendS2CPacket> CHUNK_BATCH_START = 
    ...
    // 区块delta更新（单区块变化）
    public static final PacketType<ChunkDeltaUpdateS2CPacket> SECTION_BLOCKS_UPDATE = 
...
public class PlayPackets {
    ...
    // 销毁实体
    public static final PacketType<EntitiesDestroyS2CPacket> REMOVE_ENTITIES = 
    ...
    // 实体属性（生命值等）
    public static final PacketType<EntityAttributesS2CPacket> UPDATE_ATTRIBUTES = 
...
// 这样玩家感觉操作是"即时"的
public void onPlayerInput(PlayerInputPacket packet) {
    // 立即更新本地玩家位置
    ...
    // 同时发送给服务端
    connection.send(new PlayerMoveC2SPacket(newPos.x, newPos.y, newPos.z, ...));
}
```
...
public void onPlayerMove(PlayerMoveC2SPacket packet) {
    // 检查移动是否合法
    if (isMoveValid(player, packet)) {
        // 合法：更新位置并广播
        player.setPosition(packet.getPos());
        broadcastToNearby(packet);  // 通知周围玩家
    } else {
        // 非法：回滚位置
        player.setPosition(player.getPreviousPos());
        // 发送校正包
...
sequenceDiagram
    ...
    C->>S: 发送位置更新
    S->>S: 计算需要加载的区块
    S->>C: StartChunkSendS2CPacket (批次开始)
    ...
    C->>C: 渲染新区块
    C->>S: AcknowledgeChunksC2SPacket
    Note right of C: 确认区块已加载
```

## 小结

1. **同步** = 让所有玩家看到相同的游戏世界
2. **客户端预测** = 客户端先本地处理，让操作感觉即时
3. **服务端权威** = 服务端是"真理之源"，可以纠正客户端
4. **三种同步**：
   - 位置同步：玩家和实体的位置
   - 区块同步：玩家周围的方块数据
   - 实体同步：实体的状态和属性
5. **数据包驱动** = 所有同步都通过数据包实现

## 练习

1. 在源码中查找 `PlayerPositionLookS2CPacket`，理解它如何纠正客户端位置
2. 查找 `ChunkDataS2CPacket`，了解区块数据的结构
3. 思考：为什么服务端要有"权威"？如果客户端可以随意移动会怎样？

## 相关链接

- 上一章：[第35章 协议状态机](./35-protocol-states.md)
- 下一章：[第37章 命令系统入门](../Part-7-Command/37-command-intro.md) - 了解命令系统
...
| 文件 | 源码路径 | 说明 |
|------|----------|------|
| ClientBoundEntityS2CPacket.java | `net/minecraft/network/packet/s2c/play/ClientBoundEntityS2CPacket.java` | 实体同步数据包 |
| ClientBoundBlockUpdateS2CPacket.java | `net/minecraft/network/packet/s2c/play/ClientBoundBlockUpdateS2CPacket.java` | 方块更新数据包 |
| ChunkDataS2CPacket.java | `net/minecraft/network/packet/s2c/play/ChunkDataS2CPacket.java` | 区块数据同步数据包 |

> **注意**：本文中的部分源码示例基于 CFR 反编译结果，实际源码可能略有差异。

---

**关键词**：同步机制、客户端预测、服务端权威、位置同步、区块同步、实体同步
