...
public class MagicCrystalBlock extends Block {
    ...
    protected void randomTick(BlockState state, ServerWorld world, 
        ...
        if (random.nextFloat() < 0.05f) {
            // 闪烁效果：切换亮度
            int currentLuminance = state.get(LuminousBlock.LUMINANCE);
...
| 问题 | 原因 | 解决方法 |
|------|------|----------|
| 方块不显示 | 材质路径错误 | 检查 textures 目录 |
| 亮度不变化 | 没开随机刻 | 添加 `.ticksRandomly()` |
...
| `Missing texture` | 材质文件不存在 | 检查路径是否正确 |
...
- [Block 基础](../Part-3-Block-Item/14-block-basics.md)
- [BlockState](../Part-3-Block-Item/15-block-state.md)
- [BlockEntity](../Part-3-Block-Item/16-block-entity.md)
- [资源包](../Part-8-Resource/40-resource-pack.md)
...
source/net/minecraft/block/AmethystBlock.java - 紫水晶方块参考
```
